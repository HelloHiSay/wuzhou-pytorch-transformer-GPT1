...
DEFAULT_CONFIG = dotdict({
    'n_embd': 768,       # hidden size / embedding size，模型隐藏层维度（每个 token 的向量维度）
    'n_head': 12,        # number of attention heads，多头注意力的头数
    'n_layer': 12,       # number of transformer blocks，Transformer 堆叠层数
    'embd_pdrop': 0.1,   # embedding dropout rate，token embedding + 位置 embedding 后的 dropout 概率
    'attn_pdrop': 0.1,   # attention dropout rate，注意力权重 softmax 后的 dropout 概率
    'resid_pdrop': 0.1,  # residual dropout rate，残差连接（x + sublayer）之后的 dropout 概率
    'afn': 'gelu',       # activation function，激活函数，可选 relu / swish / gelu（GPT 默认用 gelu）
    'clf_pdrop': 0.1     # classification head dropout rate，分类头（下游任务输出层）的 dropout 概率
})
